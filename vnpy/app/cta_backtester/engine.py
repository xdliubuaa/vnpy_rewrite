...
from vnpy.trader.tushareData import tushareData
from vnpy.trader.cryptoCurrency import currencyData
...
class BacktesterEngine(BaseEngine):
    ...
    def init_engine(self):
        ...
        if self.data_type == "stock_tushare":
            self.init_tusharedata()
        elif self.data_type == "stock_rq":
            self.init_rqdata()
        else:
            self.init_currency()
    ...
    def init_tusharedata(self):
        """
        Init tsData.
        """
        result = tushareData.init()
        if result:
            self.write_log("tushareData数据接口初始化成功")

    def init_currency(self):
        """
        Init currency.
        """
        result = currencyData.init()
        if result:
            self.write_log("currencyData数据初始化成功")
    ...
    def run_downloading(
        ...
        try:
            ...
            # Otherwise use RQData to query data
                ...
                self.write_log(f"[run_downloading] start..., {self.data_type}, req info:{req.symbol}-{req.exchange.value},{req.interval}") 
                if self.data_type == "stock_tushare": 
                    data = tushareData.tuquery(req)
                elif self.data_type == "stock_rq":
